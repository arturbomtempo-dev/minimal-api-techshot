...
- 🎤 **Slides da apresentação:** [minimal-apis-techshot.pdf](./presentation/minimal-apis-techshot.pdf)
  > Material usado no Techshot sobre Minimal APIs em .NET.
...
- **Domínio tipado por enums:** gênero musical (`Rock`, `Pop`, `HipHop`, `Electronic`, `Jazz`, `Classical`, `Mpb`, `Metal`, `Other`) e status de escuta (`WantToListen`, `Listening`, `Favorite`, `Archived`), serializados como texto no JSON.
...
- **Soft delete:** o `DELETE` marca o registro como removido em vez de apagar a linha, e todas as consultas ignoram os registros deletados.
...
- **Target framework:** `net10.0`, alinhado ao SDK travado no `global.json`
- **Estilo de API:** ASP.NET Core Minimal APIs, com `MapGroup` e `TypedResults`
- **Documentação:** Swashbuckle.AspNetCore `10.2.3`, com comentários XML habilitados
...
- **Provider:** Npgsql.EntityFrameworkCore.PostgreSQL `10.0.3`, com `EnableRetryOnFailure`
...
└── TuneTrail/                                 # Raiz da solution
    ...
    └── src/
        └── TuneTrail.Api/                     # Projeto da API
            ├── TuneTrail.Api.csproj           # Dependências e target framework
            ...
            │   └── Enums.cs                   # MusicGenre e ListeningStatus
            │
            ├── Data/
            │   └── Database/
            │       ├── Entities/
...
- 🎤 **[Slides do Techshot](./presentation/minimal-apis-techshot.pdf):** a apresentação usada na palestra, com a visão geral do que são Minimal APIs, quando elas fazem sentido e como se comparam ao modelo de Controllers.
...
- **A todos que acompanharam o Techshot sobre Minimal APIs**, pelas perguntas e discussões que ajudaram a moldar o conteúdo deste repositório e do tutorial.
